...
void Driver_Throttle_Init(uint16_t magnitude, Throttle_Direction_T direction) {
...
void Driver_Throttle_SetFrequency(uint16_t magnitude,
  ...
  if (direction == THROTTLE_DIRECTION_REVERSE) {
    UART.setRPM(-magnitude);
  } else {
    UART.setRPM(magnitude);
  }
