...
#include "config.h"
#include "drivers/driver_debug_led.h"
#include "drivers/driver_ebrake.h"
#include "drivers/driver_fan.h"
#include "drivers/driver_status_led.h"
#include "drivers/driver_steering.h"
#include "drivers/driver_throttle.h"
...
void App_StateMachine_Tick() {
  ...
  case (STATE_CONSOLE): {
    ...
    if (ticks_in_state == 10) {
      Driver_UART_Transmit(NUCLEO, "Enter pairs of [(-)THROTTLE,STEERING] "
                                   "values. ESC to return to IDLE.\r\n");
    }
    ...
    if (uart_seq[uart_seq_tail - 1] == 0x0D ||
      ...
      for (uint8_t char_num = 0; char_num < uart_seq_tail - 2; char_num++) {
        ...
        if (((curr_char < '0') || (curr_char > '9')) && (curr_char != ',') &&
          ...
          Driver_UART_Transmit(NUCLEO, "Invalid values entered! err1\r\n");
          // mark failure to avoid printing outside loop
          temp_data = 2;
          break;
        }
  ...
  case (STATE_AUTO): {
    ...
    if ((uart_seq_tail >= (JETSON_UART_SEQ_LENGTH + 2)) &&
        (uart_seq[uart_seq_tail - 1] == JETSON_UART_END_BYTE)) {
      // check if first byte was the start byte
      if (uart_seq[uart_seq_tail - JETSON_UART_SEQ_LENGTH - 2] ==
          JETSON_UART_START_BYTE) {
        ...
        throttle_dir =
            THROTTLE_DIRECTION_FORWARD; // always forward in autonomous
